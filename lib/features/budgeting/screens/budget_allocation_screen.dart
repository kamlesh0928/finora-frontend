...
class _BudgetAllocationScreenState extends State<BudgetAllocationScreen> {
  ...
  void initState() {
    super.initState();
    _categories =
        budgetCategoriesByRole[widget.role] ??
        budgetCategoriesByRole['Student']!;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text('Budget ${Formatters.currency(_totalBudget)}'),
      ),
      body: SafeArea(
        child: Column(
          children: [
                      ...
                      children: [
                        Row(
                          ...
                          children: [
                            ...
                            Text(
                              Formatters.currency(_allocations[i]),
                              style: theme.textTheme.titleSmall?.copyWith(
                                fontWeight: FontWeight.bold,
                                color: const Color(0xFF1565C0),
                              ),
                        ...
                        Slider(
                          ...
                          min: cat.min,
                          max: cat.max,
                          divisions: ((cat.max - cat.min) / 500).round().clamp(
                            1,
                            100,
                          ),
                        ...
                        Row(
                          ...
                          children: [
                            Text(
                              Formatters.currency(cat.min),
                              style: theme.textTheme.bodySmall?.copyWith(
                                color: theme.colorScheme.onSurfaceVariant,
                              ),
                            ),
                            Text(
                              'Recommended: ${Formatters.currency(cat.recommended)}',
                              style: theme.textTheme.bodySmall?.copyWith(
                                color: const Color(0xFF1565C0),
                              ),
                            ),
                            Text(
                              Formatters.currency(cat.max),
                              style: theme.textTheme.bodySmall?.copyWith(
                                color: theme.colorScheme.onSurfaceVariant,
                              ),
                            ),
              ...
              Padding(
                ...
                child: Container(
                  ...
                  child: Column(
                    children: [
                      Icon(
                        score >= 70 ? Icons.check_circle : Icons.info_outline,
                        color: score >= 70
                            ? const Color(0xFF2E7D32)
                      ...
                      Text(
                        score >= 70
                            ? 'You followed the 50-30-20 rule well!'
                            : 'Try allocating more to savings (aim for 20%)',
                      ...
                      ElevatedButton(
                        ...
                        child: const Text('Back to Budgeting'),
                      ),
                    ],
                  ),
                ),
  ...
  void _submitBudget(BuildContext context, int score) {
    ...
    game.completeBudgetChallenge();
    if (score >= 80) {
      wallet.credit(
        amount: 2000,
        category: 'reward',
        description: 'Budget Challenge: Excellent score ($score/100)',
        sourceModule: 'smart_budgeting',
      );
    } else if (score >= 60) {
      wallet.credit(
        amount: 1000,
        category: 'reward',
        description: 'Budget Challenge: Good score ($score/100)',
        sourceModule: 'smart_budgeting',
      );
    } else {
      wallet.debit(
        amount: 500,
        category: 'budgeting',
        description: 'Budget Challenge: Poor score ($score/100)',
        sourceModule: 'smart_budgeting',
      );
    }
  }
